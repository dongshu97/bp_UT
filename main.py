...
if args.dataset == 'mnist':
    ...
    if args.action == 'bp_Xth' or args.action == 'test':
...
# </editor-fold>
    ...
    elif args.action == 'bp_Xth' or args.action == 'test' or args.action == 'visu':
...
if __name__ == '__main__':
    ...
    # Create the network
    net = Net(args)
    if args.action =='visuEP':
        W_reverse = nn.ModuleList(None)
        for i in range(len(args.layersList)-1):
            W_reverse.append(net.W[len(args.layersList)-2-i])
        net.W = W_reverse
    ...
    if args.action == 'bp':
        ...
        DATAFRAME = initDataframe(BASE_PATH, args, net, method=args.action)
        print(DATAFRAME)
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            # testing process
            test_error_epoch = test_bp(net, args, test_loader)
            test_error.append(test_error_epoch.item())

            DATAFRAME = updateDataframe(BASE_PATH, args, DATAFRAME, net, train_error, test_error)
    ...
    # <editor-fold desc="Train with unsupervised Methods">
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            # classifying process
            response = classify(net, args, class_loader)

            # testing process
            test_error_av_epoch, test_error_max_epoch = test_Xth(net, args, test_loader, response=response, spike_record=0)

            test_error_av.append(test_error_av_epoch.item())
            test_error_max.append(test_error_max_epoch.item())
            DATAFRAME = updateDataframe(BASE_PATH, args, DATAFRAME, net, test_error_av, test_error_max)
    ...
    # <editor-fold desc="Test a training methods with little dataset">
        ...
        optimizer = torch.optim.SGD(net.parameters(), lr=args.lr[0])

        if args.batchSize == 1:
            Y_p = torch.zeros(args.layersList[-1], device=net.device)

        Xth_record = torch.zeros((1, args.layersList[-1]), device=net.device)
        Yp_record = torch.zeros((1, args.layersList[-1]), device=net.device)
        ...
        for i in range(iterations):
            ...
            # forward propagation
            output = net(args, image.to(torch.float32))

            unsupervised_targets = torch.zeros(output.size(), device=net.device)
            unsupervised_targets.scatter_(1, torch.topk(output.detach() - Xth, args.nudge_N).indices,
            ...
            if i == 0:
                ...
                Xth_record = torch.cat((Xth_record, Xth.reshape(1, args.layersList[-1])), dim=0)
                Yp_record = torch.cat((Yp_record, Y_p.reshape(1, args.layersList[-1])), dim=0)
        ...
        plot_output(Xth_record, args.layersList[-1], xlabel, 'Xth values', path_test, prefix)
        plot_output(Yp_record, args.layersList[-1], xlabel, 'Neuron activities', path_test, prefix)
    ...
    # <editor-fold desc="Analyze a trained network">
        ...
        spike, predic_spike_max, predic_spike_av = test_Xth(net, args, test_loader,
    ...
    if args.spike:
        ...
        spike, predic_spike_av, predic_spike_max = test_Xth(net, args, test_loader,
                                                            response=response, spike_record=1)
    ...
    if args.imWeights:
        ...
        for i in range(len(args.layersList)-1):
            ...
            if args.device >= 0:
                weights = weights.cpu()
            plot_imshow(weights, args.layersList[i+1], display, imShape, figName, path_imshow, prefix)

        # calculate the overlap matrix
        if len(args.layersList) > 2:
            overlap = net.W[0].weight.data
            for j in range(len(args.layersList)-2):
                overlap = torch.mm(net.W[j+1].weight.data, overlap)
            if args.device >= 0:
                overlap = overlap.cpu()
            display = args.display[-2:]
            imShape = args.imShape[0:2]
            plot_imshow(overlap, args.layersList[-1], display, imShape, 'overlap', path_imshow, prefix)
    # </editor-fold>
    ...
    if args.maximum_activation:
        ...
        for j in range(len(args.layersList)-1):
            if args.dataset == 'mnist':

                image_max = torch.zeros(args.layersList[j+1], 28*28)
                # image dimension (batch, channel, height, width)
                # image_max = torch.rand(args.layersList[j+1], 1, 28, 28, requires_grad=True, device=net.device)
            ...
            #     optimizer.step()
            #
            #     image_max = (data_average * args.layersList[j+1] * (image_max.data / torch.norm(image_max.data).item())).requires_grad_(True)
            #
            # image_max = image_max.view(image_max.size(0), -1).cpu()

            # TODO the following part is the original model

            for i in range(args.layersList[j+1]):
                #image = torch.rand(args.layersList[0], 1, requires_grad=True, device=net.device)
                if args.dataset == 'mnist':
            ...
            # calculate
            neurons_each_class = int(args.layersList[j+1]/args.n_class)

            if neurons_each_class <= 5:
                plot_imshow(image_max, args.layersList[j+1], display, imShape, figName, path_activation, prefix, all_responses[j][0])

            elif neurons_each_class <= 10:
                plot_NeachClass(image_max, args.n_class, neurons_each_class, display, imShape,
                ...
                plot_NeachClass(image_max, args.n_class, 2, display, imShape, all_responses[j][0], figName, path_activation,
                                prefix)
